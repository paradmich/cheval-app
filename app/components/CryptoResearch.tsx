...
interface Reserves {
  ...
  ethUsd: string
  totalUsd: string
  byExchange: { exchange: string; usd: string }[]
}
    ...
    <>
      ...
      {data.reserves && (
        <div className="grid two" style={{ marginBottom: 16 }}>
          <div className="card">
            <div className="ch">
              <h3>💰 Smart Money · Exchange Reserves</h3>
              <span className="muted sans">live · on-chain (CEX wallets)</span>
            </div>
            <div className="kv">
              <span className="k">Stablecoin dry powder</span>
              <span className="up">
                {data.reserves.stablecoinUsd} <span className="muted">· sidelined buying power</span>
              </span>
            </div>
            <div className="kv">
              <span className="k">ETH on exchanges</span>
              <span>
                {data.reserves.ethUsd} <span className="muted">· inventory / distribution watch</span>
              </span>
            </div>
            <div className="kv">
              <span className="k">Total tracked reserves</span>
              <span>{data.reserves.totalUsd}</span>
            </div>
            <p className="muted sans" style={{ marginTop: 10, fontSize: 11 }}>
              Outflows (falling reserves) = coins moving to custody, typically bullish; rising
              exchange balances = potential sell-side inventory.
            </p>
          </div>
          <div className="card">
            <div className="ch">
              <h3>Reserves by Exchange</h3>
              <span className="muted sans">ETH + stablecoins (USD)</span>
            </div>
            <table>
              <thead>
                <tr>
                  <th>Exchange</th>
                  <th className="r">On-chain Reserves</th>
                </tr>
              </thead>
              <tbody>
                {data.reserves.byExchange.map((e) => (
                  <tr key={e.exchange}>
                    <td>
                      <span className="sym">{e.exchange}</span>
                    </td>
                    <td className="r num">{e.usd}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <p className="muted sans" style={{ marginTop: 14, fontSize: 11 }}>
        Live: prices, 24h/7d moves &amp; market caps from CoinGecko; exchange reserves (smart-money
        dry-powder signal) from CEX on-chain wallets — both via Apify.
        Commentary &amp; signals
