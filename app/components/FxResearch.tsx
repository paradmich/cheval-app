...
/** Retail positioning cell: dominant side %, flagged when crowded (contrarian). */
function RetailCell({ long, short }: { long: number | null; short: number | null }) {
  if (long === null || short === null) return <span className="muted">—</span>
  const dominantLong = long >= short
  const pct = dominantLong ? long : short
  const label = `${pct}% ${dominantLong ? 'L' : 'S'}`
  // Crowded retail is a contrarian flag; tint gold at extremes.
  if (pct >= 70) return <span className="pill gold">{label}</span>
  return <span className="num" style={{ color: 'var(--mut)' }}>{label}</span>
}
  ...
  return (
    <>
            ...
            <tbody>
              {data.watchlist.map((row) => (
                <tr key={row.pair}>
                  ...
                  <td className="r">
                    <RetailCell long={row.retailLong} short={row.retailShort} />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <p className="muted sans" style={{ marginTop: 14, fontSize: 11 }}>
        ...
        {data.sentimentLive ? 'Retail positioning: live Myfxbook Community Outlook (contrarian). ' : ''}
        Commentary &amp; signals
